...
- **Charts — adopt `recharts`.** The dashboard (pie + bar + line) is built on `recharts`, a new approved frontend dependency. The existing `d3` is Sankey-specific and low-level; recharts is far less code for standard dashboard charts and is the right tool here. (See the "Don't add without approval" list in CLAUDE.md — this is the explicit approval, 2026-06-21.)
