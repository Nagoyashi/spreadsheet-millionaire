...
**Status (v0.15.3):** The rollout is complete — all twelve entries are `published: true`. The flag is deliberately **kept**, not removed: it no longer narrows anything, but it is the offline fallback and fresh-DB seed mirror for the runtime publish state (next section), so collapsing it would leave the fallback with no source.
...
**Trackers join the same mechanism (v0.12.1) — the build-time tracker flags are gone.** The Net Worth + Income & Expense trackers previously shipped dark behind build-time `VITE_NETWORTH_ENABLED` / `VITE_INCOME_EXPENSE_ENABLED` flags (`featureFlags.js`). That file is **deleted**; tracker visibility now rides this same runtime `calculator_publish` mechanism. `backend/publishable.py` defines `PUBLISHABLE_TYPES = VALID_CALC_TYPES + TRACKER_TYPES` (`'net-worth'`, `'income-expenses'` — slugs matching the routes); `db_init` seeds a row per publishable type (trackers defaulted **unpublished** = coming soon at introduction; since v0.15.3 the seed is `publishable.DEFAULT_PUBLISHED_PUBLISHABLE` = everything published, rollout complete — existing DB rows are never touched, seeding only backfills). The frontend's `trackers.js` exposes `useLiveTrackers()` / `useVisibleUpcoming()` (runtime hooks over `usePublishedTypes`, replacing the old static `LIVE_TRACKERS`/`VISIBLE_UPCOMING`), and the `/app/net-worth` + `/app/income-expenses` route guards check the published set instead of a flag. Net effect: **revealing a tracker out of "coming soon" is an admin toggle in /admin Overview, not a redeploy** — exactly the eventual waiting-list launch lever. This supersedes the "ships dark behind a feature flag" launch-gating notes in §§ "Net Worth Tracker" / "Income & Expense Tracker".
...
## Product analytics via PostHog (EU cloud) — the activation funnel

**TL;DR:** A first-party, EU-hosted **PostHog** SDK (`posthog-js`) instruments a discrete **activation funnel** — `calculator_used → account_created → tracker_first_entry → second_session → upgrade_viewed → upgrade_clicked`. **Key-gated** exactly like Sentry: no `VITE_POSTHOG_KEY`, no init, no SDK phones home. It is **not** autocapture analytics — every event is an explicit `capture()` call carrying only non-PII dimensions. This is *product* analytics (what users do in the app), complementary to the GA4 *traffic* proxy behind `/admin` (§ "Admin analytics — GA4 proxy"); the two don't overlap. Wiring PostHog data into the `/admin` surface is #178.

**Decision (#177):** `frontend/src/posthog.js` owns `initPostHog()` (called once in `main.jsx`, key-gated) and re-exports the static `posthog`. `frontend/src/api/analytics.js` is the single instrumentation module — one named helper per funnel event, so call sites stay declarative and event names/payloads live in one place. All helpers no-op until a client loads (`posthog.__loaded`), so callers need no guard. Emit points: `calculator_used` in `useCalculatorInputs` (once per mount, on first real edit — a ref guard, not per keystroke; loading a saved record re-seeds via `setInputs` and never fires it); `account_created` + `identify()` in `useAuth.register`, `identify()` also on login and on a returning `status` check, `reset()` on logout; `tracker_first_entry` in `useNetWorthData`/`useIncomeExpenseData` on the first successful *add* (one-shot, localStorage-guarded so a reload or second add never re-fires); `second_session` via a localStorage session counter (`analytics.trackSession()` on boot, 30-min inactivity gap, fires once at the start of session two — no backend change, since the public `/api/auth/status` deliberately omits `last_login_at`); `upgrade_viewed`/`upgrade_clicked` on the Settings-page Premium card.

**Why the Premium card exists now (before billing):** the funnel's two upgrade events need a real surface, but monetization is a later phase, so the Settings Premium card is a **minimal placeholder** — mounting fires `upgrade_viewed`, clicking fires `upgrade_clicked` and shows a stay-tuned note. It measures upgrade interest without pre-empting pricing decisions. When real checkout lands it becomes (or links to) the checkout entry.

**Privacy posture (invariant 8; PostHog is added to `PrivacyPage.jsx`'s sub-processor list):** EU cloud host (`eu.i.posthog.com`) keeps event data in the EU. `autocapture: false` and `disable_session_recording: true` are the load-bearing settings — this is a personal-finance app, and autocapture or Session Replay would hoover financial values off the page; both stay off, permanently. `capture_pageview`/`capture_pageleave: false` (no behavioural page stream), `person_profiles: 'identified_only'` (no profile for anonymous visitors). Payloads carry only `calc_type` / `tracker` / `source` — never an email or a financial input; `identify()` sends the numeric user id only. The privacy page's old blanket "no third-party analytics" line was reworded to the accurate "no session recording, no keystroke capture, no behavioural profiling — analytics is limited to aggregate events."

**Why `posthog-js` earns its place (the "justify deps" rule):** the funnel needs reliable client-side event capture with EU residency, identity stitching (anon → account), and dedup/session semantics. Hand-rolling that (a fetch-to-ingest wrapper plus session/identity bookkeeping) would re-implement the SDK badly; PostHog is the EU-hosted, privacy-configurable vendor invariant 8 prefers. It is the one new frontend dependency this ticket adds.

**Reading it back for `/admin` (#178):** the admin Analytics screen surfaces the funnel via a **server-side** read. `services/posthog_analytics.py` queries PostHog's **HogQL Query API** with a **personal API key** (`POSTHOG_API_KEY`) — a *secret*, server-side only, **never** `VITE_`-prefixed (distinct from the public browser ingest key). Transport is **stdlib `urllib`** — one POST, no new backend dependency (the "justify deps" rule again). It's gated independently from GA4: `services/analytics.get_overview` adds `posthog_configured` + `activation_funnel` (the six funnel stages) and sources `top_calculators` from PostHog's `calculator_used` events — PostHog is now the source of truth for per-calculator usage, since the GA4 `calc_run` event was never emitted (see § "Admin analytics — GA4 proxy"). A configured-but-broken PostHog surfaces in `posthog_error` (labelled, not a 500), mirroring GA4's empty-state contract. `POSTHOG_HOST` for the read defaults to the **API** host `eu.posthog.com`, not the ingest host `eu.i.posthog.com` the browser posts to.
...
**Monthly grid bulk entry (the `v0.15.0` cycle, #292).** The main bulk-input mode: pick a month, fill each category's sum, save once — one month-column of a bookkeeping spreadsheet. Decisions:

- **A grid cell is one aggregate transaction row** per (type, category, month): `source='monthly'`, `occurred_on` = the **first of the month**, in the same `ie_transactions` stream. Because an aggregate row *is* a transaction, the summary/overview/forecast layers need no second aggregation path. Rejected: a separate `ie_monthly_sums` table (every read would join/merge two sources and the by-category breakdown would need union logic); free-date aggregate rows (a fixed first-of-month convention makes the rows self-describing).
- **`source` column, server-set, single-sourced.** `source TEXT NOT NULL DEFAULT 'manual'` + CHECK against `TRANSACTION_SOURCES = ('manual', 'monthly')` in `income_expense_types.py` (the `calc_types` pattern), added via additive idempotent DDL + CHECK rebuild in `db_init.py`. The value is **set by the write path, never accepted from the request body**: `POST /transactions` writes `'manual'`, the month PUT writes `'monthly'` (`TransactionSchema` raises on unknown fields, and `source` stays out of the model's column allow-list). `'import'` joins the enum when CSV/PDF import ships (one Python edit + `db_init.py` run — this is the marker ROADMAP's Phase-16.1/#296 import paths reuse).
- **Month PUT is delete-then-insert in one transaction, not upsert.** `PUT /api/income-expense/months/<year>/<month>` replaces that month's `source='monthly'` rows wholesale: delete the bucket, insert the submitted cells, commit once. Boring beats clever (invariant 7): no `ON CONFLICT` against a synthetic uniqueness key, trivially idempotent, and "empty/zero cell = no row" falls out for free (clearing a cell is just not resubmitting it). A **partial unique index** on `(user_id, type, category, occurred_on) WHERE source = 'monthly'` guards integrity anyway. Trade-off accepted: row ids/`created_at` churn on re-save — aggregate rows carry no history worth preserving.
- **Monthly rows stay ordinary transactions.** Editable/deletable from the Transactions tab (badged "Monthly entry" there — #294). The grid GET buckets by **month range**, not the exact first-of-month date, so a date-edited monthly row still shows in its month; if edits ever produce two monthly rows for one cell, the GET sums them and the next month-save collapses them back to one. No recurrence and no note on aggregate rows (recurrence on a sum makes no sense; a per-cell note can be added later if wanted).
- **The grid GET also returns the month's per-(type, category) sums of `source='manual'` rows**, read-only, so the UI can show what individual transactions already cover — the user isn't tempted to double-enter.

**Custom categories (`v0.15.1`).** The curated-enum decision above is **superseded**: real bookkeeping needs the user's own categories (the owner's spreadsheet has "Wohnung", "Strom + Gas", "Reisekosten"…). Decisions:

- **User-scoped `ie_categories` table** — `(user_id, type, key, name, archived)`. `key` is an immutable server-generated slug **stored on transaction rows** (`ie_transactions.category` keeps holding keys); `name` is display-only. Defaults are **lazily seeded per user** from `income_expense_types.DEFAULT_CATEGORIES` with keys equal to the old curated slugs, so every historical transaction resolves against a seeded row and no data migration is needed.
- **Validity moves from schema/CHECK to the model layer.** A static Marshmallow `OneOf` / DB CHECK can't see a per-user set, so the routes check writes against `active_category_keys(user_id, type)` (transactions, grid cells) and the old union CHECK was rebuilt into a plain length bound. Marshmallow still bounds the string's shape.
- **Archive is a soft delete, restore is first-class.** Archiving hides a category from pickers and the grid but **history keeps aggregating** (rows keep their key; summaries/breakdowns are source-of-truth on `ie_transactions`). Restore via `PATCH {archived: false}` — and **POSTing a name that case-insensitively matches an archived category restores that row instead of creating a duplicate** (unique index on `(user_id, type, lower(name))` makes the collision impossible even racing). Rejected alternatives: hard delete (destroys financial history), block-until-recategorized (hostile busywork), rename (deferred — `key` immutability makes it safe to add later).
- **The grid respects the archive boundary.** The month PUT's wholesale delete is scoped to the user's **active** categories, so an archived category's aggregate rows survive a re-save (rendered read-only in the grid; restore to edit). Submitting a cell for an archived/unknown category is a 422.
- **Slug collisions get a numeric suffix** (`caf`, `caf-2`) — keys are never reused across categories because old rows point at them forever.
- **Rename, reorder, cap (`v0.15.2`).** Rename changes `name` only — the immutable `key` is exactly why it's safe (every historical row keeps resolving; a rename conflicting case-insensitively with another category 409s). Drag order lives in a `position` column per (user, type); the reorder endpoint takes the FULL id list of a type and rejects a stale/partial list wholesale rather than partially applying. Active categories are capped at `CATEGORY_ACTIVE_LIMIT` (20) per type — a grid with more rows stops being scannable; archived rows don't count, and a restore past the cap is blocked like a create. Per-month category scoping was considered and rejected (owner decision, 2026-07-15): categories stay one global list because they define how data aggregates across months/years — clutter is solved by the cap, the collapsed Archived list, and drag order instead.
- **Recurring transactions in the grid: read-only + explicit Apply (`v0.15.2`).** The selected month shows "+ $X recurring" per category (client-side projection, `recurringByCategoryForMonth` — occurrences strictly after the anchor, so the anchor month isn't double-shown next to its own manual sum). An "Apply recurring to empty fields" button copies the projections into EMPTY inputs only. Auto-prefilling editable fields was rejected: one accidental "Save month" would record rows the user never typed.
- `ie_categories` joined `USER_SCOPED_TABLES` (deletion cascade + export) the moment it was created — the drift guard enforces this.
...
**TL;DR:** Every calculator numeric field passes `min`/`max` and `NumInput` clamps in JS — `max` on change, `min` on blur. The robustness fix lives in shared code (`NumInput` + `fmt` + one `finiteOr` helper); per-calculator work is just passing bounds.

**Decision:** Numeric inputs are made robust to bad input centrally, so all calculators inherit it:
- **`NumInput` clamps `max` on change, `min` on blur** (amended v0.14.4 — originally both clamped on change). Typing appends digits, so a prefix of an in-range number can never overshoot `max` — snapping immediately is safe. But prefixes *are* below a raised `min` ("3" on the way to "35" with `min=18`), so the original clamp-per-keystroke made age fields untypeable: the field snapped to 18, then "185" overshot `max` and snapped again — "numbers jumping around." Under-min values now sit while the field is focused and snap on blur; negatives into a non-negative range still snap on change (they can't grow into range). The guarantee is now on the *committed* value: a blurred field is never outside `[min, max]`. The flaw went unnoticed because the original four published calculators only use `min={0}` — it surfaced when the age-bounded calculators (Barista/Coast FIRE) were published. Non-numeric input is ignored, empty stays empty (calculators read `""` as `0`). Native `min`/`max`/`step` are kept for the spinner and a11y but are *not* relied on — they don't stop typed or pasted values like `8e31` or `-5`, which was the actual failure mode.
...
**Comma decimals (`v0.15.2`) — `NumInput` is a text input now.** European users type `1,5`; a `type="number"` input swallows the comma keystroke before JS ever sees it, so the input switched to `type="text"` + `inputMode="decimal"` (numeric keyboard on mobile preserved) and `handleChange` normalises the first `,` to `.` before the existing parse/clamp pipeline — every consumer keeps reading the value with `Number()`. The clamp semantics above are unchanged; the native spinner and native `min`/`max` are gone, which loses nothing (the JS clamps were always the real guard, and the committed-value guarantee holds). Trade-off accepted: the field displays the normalised dot, not the typed comma — simpler than locale-aware display, and `fmt()` remains the only formatter (the i18n note above still owns real localisation).
...
- **DB tests skip without `TEST_DATABASE_URL` — except in CI (#183).** The DB-free majority (incl. the `/api/health` smoke test) needs no infrastructure, so a bare `cd backend && pytest` is green on any checkout. DB-backed suites (auth, IDOR, saved-data, deletion/export) opt in by pointing `TEST_DATABASE_URL` at a disposable database — wired into CI by the CI workflow. In CI, `CI_REQUIRE_DB=1` turns a missing `TEST_DATABASE_URL` into a **hard failure** instead of a skip: a broken service container must never let the DB-backed majority silently vanish while CI stays green. Locally the env var is unset, so the skip convenience is unchanged.
