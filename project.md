...
| `v0.14.3` | 2026-07-14 | Calculator math fixes from the prod-wide QA sweep (#279): Compound Interest's inverted compounding-frequency ratio, Withdrawal Plan's real-return/balance-at-horizon/depleted-display trio, Coast FIRE's static-vs-rising coast threshold, FIRE's "0 yrs" on unreachable goals; plus input clamps (negative loan, negative Sankey amounts, NaN%) and label-association a11y across all `NumInput`s. 18 new closed-form regression tests. → [v0.14.3](docs/releases/v0.14.3.md) |
