...
import { CATEGORY_OPTIONS, MONTH_NAMES } from './incomeExpenseOptions'
...
// read-only so nothing gets double-entered. Reads go straight through the API
// module (the month state is panel-local); saves go through the hook's
// saveMonth so the page summary refetches. See DECISIONS.md § "Income &
// Expense Tracker" (Monthly grid bulk entry).

const money = (n) => fmt(n, { compact: false })
const keyOf = (type, category) => `${type}:${category}`

const SECTIONS = [
  { type: 'income', title: 'Income', accent: 'text-emerald-600' },
  { type: 'expense', title: 'Expenses', accent: 'text-rose-600' },
]
...
export default function MonthlyEntryPanel({ availableYears, onSaveMonth }) {
  const now = new Date()
  ...
  const totals = useMemo(() => {
    const sum = (type) =>
      CATEGORY_OPTIONS[type].reduce(
        (acc, o) => acc + (Number(amounts[keyOf(type, o.value)]) || 0),
        0
      )
  ...
  async function handleSave() {
    ...
    for (const { type } of SECTIONS) {
      for (const o of CATEGORY_OPTIONS[type]) {
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <>
          ...
          >
            {SECTIONS.map(({ type, title, accent }) => (
