...
// Per-month income, summarised over the months that have any recorded activity
// (income OR expense) — so trailing empty/future months don't drag a partial
// year toward zero. Median is the more honest "typical month" than the mean when
// a few big one-off months skew the average.
export function monthlyIncomeStats(byMonth = []) {
  ...
  const incomes = active.map((m) => m.income)
  const average = incomes.reduce((sum, n) => sum + n, 0) / incomes.length
  return { average, median: median(incomes), activeMonths: active.length }
}
